/**
 * Reset Script — SupermaxPanel
 * Usage:  node scripts/reset.js
 *
 * 1. Deletes ALL data from every collection EXCEPT the superadmin Admin record.
 * 2. Re-seeds fresh data for that superadmin (employees, work entries,
 *    advance payments, orders, customers, products, categories,
 *    factory expenses, raw materials).
 */
...
const AdvancePayment = models.AdvancePayment || model("AdvancePayment", new Schema({
  userId:   { type: ObjectId, ref: "Admin", required: true, index: true },
  employee: { type: ObjectId, ref: "Employee", required: true },
  type:     { type: String, enum: ["ADVANCE","SALARY_PAYMENT"], required: true },
...
async function reset() {
  ...
  /* ── 1. Find superadmin (must already exist) ── */
  const admin = await Admin.findOne({ role: "superadmin" });
  if (!admin) {
    console.error("❌ No superadmin found in DB. Cannot proceed.");
    process.exit(1);
  }
  console.log(`👤 Superadmin found  →  ${admin.email}`);
  const userId = admin._id;

  /* ── 2. Delete ALL non-superadmin Admin records ── */
  const adminDelResult = await Admin.deleteMany({ role: { $ne: "superadmin" } });
  console.log(`🗑  Sub-admins deleted →  ${adminDelResult.deletedCount}`);

  /* ── 3. Wipe every other collection ── */
  const [c1, c2, c3, c4, c5, c6, c7, c8, c9] = await Promise.all([
    ...
    AdvancePayment.deleteMany({}),
    FactoryExpense.deleteMany({}),
    RawMaterial.deleteMany({}),
  ]);
  console.log(`🗑  Collections cleared:`);
  console.log(`     Categories:       ${c1.deletedCount}`);
  console.log(`     Products:         ${c2.deletedCount}`);
  console.log(`     Customers:        ${c3.deletedCount}`);
  console.log(`     Orders:           ${c4.deletedCount}`);
  console.log(`     Employees:        ${c5.deletedCount}`);
  console.log(`     WorkEntries:      ${c6.deletedCount}`);
  console.log(`     AdvancePayments:  ${c7.deletedCount}`);
  console.log(`     FactoryExpenses:  ${c8.deletedCount}`);
  console.log(`     RawMaterials:     ${c9.deletedCount}`);
  ...
  for (const def of employeeDefs) {
    employees.push(await Employee.create({
      userId, ...def,
      address: `${rand(1,50)} Labour Colony, Surat`,
      advancePayment: 0,
      paidPayment: 0,
      workEntries: [],
    }));
  ...
  for (const emp of employees) {
    for (let i = 29; i >= 0; i--) {
      ...
      const isOff    = isSunday || Math.random() < 0.12;
      const entry = await WorkEntry.create({
        userId,
        employee: emp._id,
        date,
        status:   isOff ? "WORK_OFF" : "WORK",
        quantity: isOff ? undefined : rand(20, 80),
        amount:   isOff ? undefined : rand(150, 900),
      });
  ...
  for (const emp of employees) {
    ...
    for (let i = 0; i < numPayments; i++) {
      const type   = pick(["ADVANCE", "SALARY_PAYMENT"]);
      const amount = type === "ADVANCE" ? rand(500, 3000) : rand(2000, 8000);
      ...
      await AdvancePayment.create({
        ...
        type,
        amount,
        note: type === "ADVANCE" ? "Cash advance" : "Monthly salary payment",
        date,
      });
      if (type === "ADVANCE") totalAdvance += amount;
      else                    totalSalary  += amount;
      apCount++;
    }
    /* reflect totals on the employee record */
    await Employee.findByIdAndUpdate(emp._id, {
      $inc: { advancePayment: totalAdvance, paidPayment: totalSalary },
    });
  }
  console.log(`💰 Advance pays   →  ${apCount}`);

  /* ── 11. FACTORY EXPENSES ── */
  const expenseNames = [
    "Electricity Bill","Water Bill","Machine Maintenance","Generator Fuel",
    "Rent Payment","Security Guard Salary","Cleaning Supplies","Office Supplies",
    "Internet Bill","Telephone Bill","Pest Control","AC Servicing",
    "Spare Parts","Tool Replacement","Safety Equipment","First Aid Kit",
    "Canteen Supplies","Drinking Water","Firefighting Equipment","Packaging Material",
  ];
  for (let i = 0; i < 20; i++) {
    await FactoryExpense.create({
      userId,
      name:        expenseNames[i],
      amount:      rand(500, 15000),
      entryDate:   thisMonth(),
      entryPerson: pick(["Deepak Verma","Rajesh Shah","Priti Doshi"]),
      shopName:    pick(["City Electric","Metro Store","Ram Hardware","Shree Suppliers"]),
      quantity:    rand(1, 20),
      status:      pick(["pending","paid","paid"]),
    });
  }
  ...
  /* ── 12. RAW MATERIALS ── */
  const materialNames = [
    "Cotton Yarn","Polyester Thread","Nylon Rope","Elastic Band","Rubber Sheet",
    "Steel Wire","Plastic Granules","Dye Powder Blue","Dye Powder Red","Dye Powder Yellow",
    "Chemical Fixative","Fabric Softener","Wax Block","Foam Sheet","Canvas Roll",
    "Jute Fiber","Synthetic Resin","Adhesive Glue","Packing Tape","Bubble Wrap",
  ];
  for (let i = 0; i < 20; i++) {
    await RawMaterial.create({
      userId,
      materialName: materialNames[i],
      shopName:     pick(["Raj Traders","Shree Raw Mart","Global Supplies","Om Store"]),
      buyerName:    pick(["Deepak Verma","Suresh Patel","Ramesh Yadav"]),
      quantity:     rand(5, 100),
      amount:       rand(800, 20000),
      date:         thisMonth(),
      status:       pick(["pending","paid","paid"]),
    });
  }
  ...
  console.log(`    Superadmin preserved  →  ${admin.email}`);
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
